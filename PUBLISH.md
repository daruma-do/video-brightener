...
3. **ポップアップUIの全体**
4. **機能イメージ図**（暗いシーン → ガンマ補正 → 視認性改善 のビジュアル）

⚠ Yellow Argon（キーワードスパム）回避のため、「対応サイト一覧」のようなブランド名のみを羅列したスクリーンショット／文面は使わない。
...
⚠ Yellow Argon（キーワードスパム）回避のため、「■ 対応サイト」見出しでサービス名を羅列するセクションは作らない。対応サイトは host_permissions でユーザに表示されるため、本文での明示は最小限（文脈内で1〜2回まで）に留める。
...
| 審査リジェクト（Yellow Argon / キーワードスパム） | 説明文で対応サイトを箇条書きや見出しで列挙しない。文脈内に最小限の言及にとどめる |
